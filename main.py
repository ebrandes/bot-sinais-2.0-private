...
def Calcula_Valor_Ciclo(lucro):
    ...
    if lucro < 0 and ciclos_ativos <= int(config['ciclos_nivel']):
        valor_entrada_ciclo = (
            valor_entrada * (float(config['valorGale']) ** int(config['niveis']))) * float(config['valorGale'])
        ciclos_ativos += 1
    else:
        valor_entrada_ciclo = float(config['entrada'])
        ciclos_ativos = 0
